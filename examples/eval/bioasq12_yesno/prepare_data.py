...
def bioasq_datareader(
    originalFilePath="resources", 
    version=VERSION):
    """
    ...
    with open(os.path.join(originalFilePath, f"training{version}b_new.json"), "r") as fp:
        train_raw_data = json.load(fp)["questions"]

    file_list = os.listdir(originalFilePath)
    test_file_list = [filename for filename in file_list if filename.startswith(str(VERSION)) and filename.find("golden") != -1]
...
def bioasq_select_qtype(
    train_raw_data, test_data,
    q_type="yesno",
    ):

    train_raw_data = [qas for qas in train_raw_data if qas[u'type']==q_type]
    test_data = [qas for qas in test_data if qas[u'type']==q_type]
...
def bioasq_sanity_check(
    ...
    return {
        ...
        "is_valid": not (train_dev_overlap or train_test_overlap or dev_test_overlap) and total_unique_ids == total_expected
    }
...
def main(args):
    ...
    train_raw_data, test_data = bioasq_datareader(originalFilePath=args[0], version=VERSION)
    train_raw_data, test_data = bioasq_select_qtype(train_raw_data, test_data, q_type="yesno")
